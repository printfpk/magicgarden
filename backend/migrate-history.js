...
async function migrateHistory() {
  try {
    ...
    // Find the 'Indian history' chapter in mongo
    const chapter = await db.collection('chapters').findOne({ title: 'Indian history' });
    if (!chapter) {
      console.log("Indian history chapter not found in mongo!");
      return;
    }
    console.log("Found Indian history chapter in Mongo:", chapter._id);
    ...
    console.log("Migration of Indian history complete!");
  } catch (e) {
    console.error(e);
  } finally {
